    ...
    def write_category_indexes
      if self.layouts.key? 'category'
        ...
        self.categories.keys.each do |category|
          slugged = category.downcase.strip.gsub(' ', '-')
          self.write_category_index(File.join(dir, slugged), category)
  ...
  module Filters
    ...
    def category_links(categories)
      categories = categories.sort!.map do |item|
        slugged = item.downcase.strip.gsub(' ', '-')
        '<strong><a href="/c/'+slugged+'/">'+item+'</a></strong>'
      end
